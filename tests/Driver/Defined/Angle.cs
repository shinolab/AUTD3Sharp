    ...
    {
        {
            ...
            AssertNearVec3(Vector3.UnitY, autd.Geometry[0][0].XDirection);
            AssertNearVec3(-Vector3.UnitX, autd.Geometry[0][0].YDirection);
            AssertNearVec3(Vector3.UnitZ, autd.Geometry[0][0].ZDirection);
        }
        {
            var autd = await Open(EulerAngles.ZYZ(0 * deg, 90 * deg, 0 * deg));
            AssertNearVec3(-Vector3.UnitZ, autd.Geometry[0][0].XDirection);
            AssertNearVec3(Vector3.UnitY, autd.Geometry[0][0].YDirection);
            AssertNearVec3(Vector3.UnitX, autd.Geometry[0][0].ZDirection);
        }
        {
        ...
        }
        {
            ...
            AssertNearVec3(Vector3.UnitY, autd.Geometry[0][0].XDirection);
            AssertNearVec3(Vector3.UnitZ, autd.Geometry[0][0].YDirection);
            AssertNearVec3(Vector3.UnitX, autd.Geometry[0][0].ZDirection);
        }
        {
            ...
            AssertNearVec3(-Vector3.UnitZ, autd.Geometry[0][0].XDirection);
            AssertNearVec3(-Vector3.UnitX, autd.Geometry[0][0].YDirection);
            AssertNearVec3(Vector3.UnitY, autd.Geometry[0][0].ZDirection);
        }
        return;

        async Task<Controller<Audit>> Open(Quaternion q) =>
            await new ControllerBuilder().AddDevice(new AUTD3(Vector3.Zero).WithRotation(q))
