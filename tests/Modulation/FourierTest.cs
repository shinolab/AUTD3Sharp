...
public class FourierTest
{
    ...
    public async Task FourierExactFloat()
    {
        ...
        {
            var m = (new Sine(50.0 * Hz) + new Sine(100.0 * Hz)).AddComponent(new Sine(150.0 * Hz))
              .AddComponentsFromIter(new[] { 200.0 * Hz }.Select(x => new Sine(x))) + new Sine(250.0 * Hz);
            Assert.Equal(LoopBehavior.Infinite, m.LoopBehavior);
            await autd.SendAsync(m);
            foreach (var dev in autd.Geometry)
            {
    ...
    public async Task FourierNearest()
    {
        ...
        {
            var m = Sine.WithFreqNearest(50.0 * Hz) + Sine.WithFreqNearest(100.0 * Hz);
            Assert.Equal(LoopBehavior.Infinite, m.LoopBehavior);
            await autd.SendAsync(m);
            foreach (var dev in autd.Geometry)
            {
