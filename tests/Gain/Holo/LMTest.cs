...
public class LMTest
{
    ...
    public async Task LM()
    {
        ...
        var g = new LM<NalgebraBackend>(backend, new float[] { -40, 40 }.Select(x => (autd.Geometry.Center + new Vector3(x, 0, 150), 5e3f * Pa)))
            .WithEps1(1e-3f)
            .WithEps2(1e-3f)
            .WithTau(1e-3f)
            .WithKMax(5)
...
#pragma warning disable CS8602, CS8605
        ...
        var backend = new NalgebraBackend();
        var g = new LM<NalgebraBackend>(backend, [(autd.Geometry.Center, 5e3f * Pa), (autd.Geometry.Center, 5e3f * Pa)]);
