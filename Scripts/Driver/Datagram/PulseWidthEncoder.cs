...
namespace AUTD3Sharp
{
    public sealed class PulseWidthEncoder : IDatagram
    {
        ...
        private delegate NativeMethods.PulseWidth PulseWidthEncoderDelegate(ConstPtr context, GeometryPtr geometryPtr, ushort devIdx, byte idx);
        ...
        public PulseWidthEncoder(Func<Device, Func<Intensity, PulseWidth>> f)
        {
            ...
            _f = (_, geometryPtr, devIdx, idx) => cache.GetOrAdd(devIdx, f(new Device(devIdx, geometryPtr)))(new Intensity(idx)).ToNative();
        }
